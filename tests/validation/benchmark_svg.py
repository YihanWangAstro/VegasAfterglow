"""Generate the README benchmark performance SVGs — one per radiation mode.

Run manually after a benchmark refresh:
    python tests/validation/benchmark_svg.py
...
def _make_chart(bench, rad_type, rad_label, out_path, theme="adaptive"):
    ...
    emit('  <text x="475" y="57" text-anchor="middle" font-size="9" class="cs">'
...
if __name__ == "__main__":
    ...
    parser.add_argument("json_path", nargs="?",
                        default=str(Path(__file__).parent / "benchmark" / "results" / "benchmark_history.json"),
