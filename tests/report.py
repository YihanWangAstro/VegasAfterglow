...
def golden_plots_section(plots_dir):
    """Embed the golden baseline-vs-run comparison figures saved by the pytest run."""
    pngs = sorted(Path(plots_dir).glob("*.png")) if Path(plots_dir).is_dir() else []
    if not pngs:
        return ""
    cards = []
    for png in pngs:
        b64 = base64.b64encode(png.read_bytes()).decode()
        stamp = datetime.fromtimestamp(png.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
        cards.append(
            f'<details class="golden-fig"><summary>{esc(png.stem)}'
            f'<span class="sec-sub"> rendered {stamp}</span></summary>'
            f'<img alt="golden comparison: {esc(png.stem)}" '
            f'style="max-width:100%" src="data:image/png;base64,{b64}"></details>')
    return (f'<section id="golden-plots"><div class="sec-head"><h2>Golden baselines '
            f'<span class="sec-sub">stored baseline (solid) vs this run (dashed); the lower '
            f'panels show the deviation in units of the acceptance tolerance — above the '
            f'dotted line is a failing bin</span></h2></div>{"".join(cards)}</section>')
...
def main():
    ...
    ap.add_argument("--golden-plots",
                    default="tests/python/golden/plots",
                    help="directory of golden comparison PNGs saved by the pytest run")
    ...
    golden_html = golden_plots_section(args.golden_plots)
...
<div class="tile"><div class="k">suite runtime</div><div class="v">{fmt_t(total_time)}</div>
  <div class="s">unit + physics tests</div></div>
    ...
    if golden_html:
        nav.append('<a href="#golden-plots">Golden plots</a>')
    if val_html:
