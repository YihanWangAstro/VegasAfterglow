...
STANDARD_PARAMS = {
    ...
    "eps_e": 0.01, "eps_B": 0.01, "p": 2.2, "xi_e": 1.0, "lumi_dist": 1e28, "z": 1.0,
}
...
def fit_powerlaw(t, f, window=5):
    ...
    A narrow moving-average window smooths out local noise before taking the mean.
    """
    ...
    if len(local_slopes) > 1:
        local_slopes = local_slopes[1:]  # skip first point (boundary effect)
    if len(local_slopes) >= window:
        kernel = np.ones(window) / window
        local_slopes = np.convolve(local_slopes, kernel, mode="valid")
    return np.mean(local_slopes)
