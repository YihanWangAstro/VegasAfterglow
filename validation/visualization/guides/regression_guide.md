...
Same layout as forward shock, with separate pages for thin-shell and thick-shell configurations. The crossing phase is highlighted distinctly from the post-crossing and deep Newtonian phases.
...
- Upper: $F_\nu$ vs $\nu$ with break frequencies marked as vertical lines
- Lower: Spectral index $\beta = d(\log F_\nu)/d(\log \nu)$
