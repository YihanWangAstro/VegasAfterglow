...
- **MCMC fitting is 3-4x faster than v2.0.5** (on-axis tophat fit: 2.2 ms → 0.6 ms per likelihood; gaussian jet with reverse shock: 29 ms → 7 ms), from the grid and observer improvements below. Walker retuning adds ~25% on top
- **Sharper angular and temporal grids**: off-axis azimuthal integration over the mirror-symmetric half range (~2x off-axis speed); refinement now tracks the Doppler structure, fixing under-resolved viewing angles near sharp jet edges (several percent → below one percent); time grid concentrated around the deceleration turnover
- **Lower default resolution, per mode**: (0.06, 0.15, 6) for forward-shock-only runs, (0.06, 0.2, 10) with a reverse shock — 15-35% faster at unchanged worst-case accuracy, calibrated per emission component across jet families and viewing angles. An explicit `resolutions` is honored as given
- **Observer-layer performance**: off-axis flux evaluations 10-20% faster with exact results preserved; reverse-shock runs reuse the forward-shock equal-arrival-time grid (~5% faster, bit-identical); multi-band series data ~10% faster
- **MCMC: twice the walkers, half the steps**: the automatic walker count doubles to fill the likelihood thread pool (~25% higher throughput; `nwalkers=...` still overrides). **Halve `nsteps` and `nburn` in existing scripts** for the same posterior volume at unchanged runtime. Default moves now `DEMove 0.9 / DESnookerMove 0.1` (~20% shorter autocorrelation times)
- **Magnetized reverse-shock accuracy**: `sigma0 > 0` shells automatically tighten the ODE tolerance (the previous uniform tolerance carried ~1% flux error at sigma ~ 1); new golden baselines at sigma = 1 and 10
- **Opt-in fast math** (`AFTERGLOW_FAST_MATH`, still off by default): ~1000x lower kernel error with full domain handling, now covering all `log2`/`exp2` call sites; ~1.2x on off-axis structured jets when enabled
- Radiative efficiency follows the analytic form everywhere (hard truncation removed); generic `Ejecta`/`Medium` constructors validate their inputs
- Validation suite unified under `tests/validation/`; the release PDF replaced by the published HTML report at [reports/latest](https://yihanwangastro.github.io/VegasAfterglow/reports/latest/)

### Removed

- **`cmb_cooling`** removed from `Radiation`, `Fitter`, and the CLI: IC cooling off the CMB is negligible for GRB afterglow shocks

### Fixed

- **Radiative losses in the blast-wave dynamics were underestimated** (all releases since v1.0.0): the dynamics-side cooling Lorentz factor missed the 8π in `B'² = 8π ε_B e`, placing the cooling break ~25x too high, so the fireball evolved too adiabatically. Late-time light curves now dim by ~10-30% at `eps_e = 0.1` (up to ~70% at `eps_e = 0.3`); fits calibrated against earlier releases should be re-run. Golden baselines regenerated; all closure-relation and crossing-scaling checks pass
- **Reverse-shock robustness**: rates evaluated on the physical domain (no more solver runaways corrupting grid rows); crossing end located by bisection instead of at grid times (removes up to one-grid-step bias near the crossing peak); magnetized (`sigma0 > 0`) shell crossing gated on shell penetration, fixing a forward-shock flux runaway
- **Numerical precision**: cancellation-free forms for the crossing rate, relative Lorentz factor, and relativistic kinematics helpers — results independent of platform FMA/SIMD behavior; magnetized jump-condition cubic solved by root deflation

### Added

- **`radiative_fireball` switch** on `Model`, `Fitter`, and the CLI (`--adiabatic`): set `False` for the adiabatic approximation used by most afterglow codes — for cross-code comparisons or low-`eps_e` sources. Default keeps the radiative dynamics
- **Unified test framework** (`make test`, see `TESTING.md`): C++ units, Python suite, and full physics validation in one run with a self-contained HTML report; closure relations, golden baselines, parameter-corner sweeps, and per-configuration timing
- CI on Linux/macOS/Windows plus a weekly validation run refreshing the published report
