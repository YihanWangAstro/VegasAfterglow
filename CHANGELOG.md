...
- **Observer-layer performance**: off-axis flux evaluations are 10-20% faster, reverse-shock runs reuse the forward-shock equal-arrival-time grid (~5%), and multi-band series evaluation is ~10% faster — none of these change results
...
- **Magnetized reverse-shock accuracy**: `sigma0 > 0` shells automatically tighten the ODE tolerance (the previous uniform tolerance carried ~1% flux error at sigma ~ 1)
- **Opt-in fast math** (`AFTERGLOW_FAST_MATH`, still off by default): the fast math kernels are ~1000x more accurate, handle edge-case inputs correctly, and now cover all `log2`/`exp2` call sites; enabling gives ~1.2x on off-axis structured jets
...
- **Numerical precision**: cancellation-free forms for the crossing rate, relative Lorentz factor, and relativistic kinematics helpers, plus a more robust magnetized jump-condition solve — results are now reproducible across platforms and compilers
