...
where :math:`\tilde{Y}` is the Compton Y-parameter that accounts for the synchrotron self-Compton (SSC) cooling:

.. math::

    \tilde{Y} = Y_{\rm SSC} = \frac{1}{2}\left(\sqrt{1 + 4\eta_e\frac{\epsilon_e}{\epsilon_B}} - 1\right)

where :math:`\eta_e` is the radiative efficiency of the electrons.
